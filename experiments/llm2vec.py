"""
The script is adapted from https://github.com/McGill-NLP/llm2vec/blob/main/llm2vec/llm2vec.py
"""
...
class LLM2Vec(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        gguf_file = backward_config.gguf_file

        config = AutoConfig.from_pretrained(base_model_name_or_path, gguf_file=gguf_file)
        config.attention_dropout = kwargs.pop("attention_dropout", config.attention_dropout)
        config.update(backward_config.to_dict())
        ...
        if hasattr(model, "init_backward"):
            model.init_backward()
        ...
        if os.path.exists(f"{config_addr}/llm2vec_config.json"):
            ...
            if llm2vec_config.backward_type != "NONE" and \
               (llm2vec_config.backward_layers != model.config.backward_layers or \
                llm2vec_config.noncausal_layers != model.config.noncausal_layers or \
                llm2vec_config.num_backward_layers != model.config.num_backward_layers or \
                llm2vec_config.num_noncausal_layers != model.config.num_noncausal_layers):
                raise ValueError("Inconsistent with the pretrained parameters.")
    ...
    def save(self, output_path, merge_before_save=False, save_config=True):
        ...
        llm2vec_config = {
            ...
            "backward_type": self.config.backward_type,
            "num_backward_layers": self.config.num_backward_layers,
            "num_noncausal_layers": self.config.num_noncausal_layers,
            "backward_layers": self.config.backward_layers,
            "noncausal_layers": self.config.noncausal_layers,
        }
