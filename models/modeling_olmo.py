...
from transformers.modeling_outputs import BaseModelOutputWithPast, TokenClassifierOutput
...
class OlmoModel(OlmoPreTrainedModel):
    ...
    def __init__(self, config: OlmoConfig):
        ...
        if self.architecture == "EXTEND":
            ...
            self.layers.extend([OlmoDecoderLayer(config, config.num_hidden_layers + layer_idx) for layer_idx in range(num_converted_layers)])
...
class OlmoForTokenClassification(OlmoPreTrainedModel):
